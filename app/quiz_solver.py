...
async def solve_single_quiz(
    ...
    try:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # CRITICAL: Check if LLM returned the example payload structure
            if isinstance(answer, dict) and all(k in answer for k in ["email", "secret", "url", "answer"]):
                print(f"[QUIZ] ⚠️  LLM returned example payload, extracting nested answer...")
                print(f"[QUIZ] Full LLM response: {answer}")
                
                # Extract the nested answer field
                nested_answer = answer.get("answer")
                
                # If nested answer is also a placeholder, we need to re-prompt
                if isinstance(nested_answer, str) and any(x in nested_answer.lower() for x in ["placeholder", "anything", "your", "example"]):
                    print(f"[QUIZ] Nested answer '{nested_answer}' is also a placeholder, need better extraction")
                    result["error"] = "LLM returned placeholder values, cannot determine real answer"
                    continue
